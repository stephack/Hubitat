/**
 *  Sonos Playlist Controller Child
 *
...
definition(
    name: "Sonos PlayList Controller Child",
    namespace: "stephack",
    author: "Stephan Hackett",
    description: "Autoplay Stations/Playlists on Sonos speakers",
    category: "My Apps",
    parent: "stephack:Sonos PlayList Controller",
...
def isInitial(){
    if(!state.plIndex) {state.plIndex=0}
...
def mainPage(){
	dynamicPage(name:"mainPage",install:true, uninstall:true){
        ...
        if(sonos){
            section("\n\nStep 2: Configure Your Playlists"){
                	href "configPlPage", title: "Configure/Save Playlists"
          	}
            /*
        		for(i in 1..vbTotal) {
...
def configPlPage(){
    dynamicPage(name: "configPlPage", title: "CONFIGURE Playlists:") {
	    for(i in 1..10){
     		section("${i}"){
                input("plLabel${i}", "text", title: "Name of Playlist:")
            	input("plSwitch${i}", "capability.switch", title: "Choose a Virtual Switch (Momenatary)")
            	input("uri${i}", "text", title: "Playlist URI:")
            }
            
        }
        section("Get URI's"){
        	paragraph "teetetet"
            input("importRecent", "button", title: "Refresh Recent Playlists")
      	}
	}
}

def appButtonHandler(btn) {
    if(btn=="importRecent"){stationSelections()}
}
/*
def confirmOptions(){
...
def initialize() {
    state.PLmap = [:]
    def test1 = settings.findAll{it.key.contains("plSwitch")}
    test1.each{
    	def index = (it.key - "plSwitch").toInteger()
        state.PLmap << ["${it.value.deviceId}" : app."uri${index}"]    
    }
    
    def virts = settings.findAll{it.key.contains("plSwitch")}
    virts.each{
...
def switchHandler(evt){
    ...
    def uri = state.PLmap.find{it.key == "${evt.deviceId}"}.value
    //def index = settings.find{it.value == evt.deviceId}.key - "plLabel"
    log.debug uri
}
