...
//    page(name: "ccPage") ///NEW ADDITION*************************************************************************************************************************************************
	
}
/*/*****************************************************************************************************************************************************************************************
def ccPage(){
    dynamicPage(name: "ccPage", title: "ccpage stuff") {
        section(""){
            input "ccName", "text", title: "Command Name:", required: true, description: "case sensitive command name eg 'testCommand'", submitOnChange: true
            input "ccDevTypes", "enum", title: "Applies to these device types:", required: true, description: "Choose device types that can use this command", submitOnChange: true, options: getDevTypes(), multiple: true
            input "ccNum", "number", title: "Number of Parameters", required: true, description: "How many parameters does this command accept", submitOnChange: true
            if(ccNum){
                for(i in 1..ccNum){
                    input "ccPtype${i}", "enum", title: "Parameter ${i} Type", required: true, description: "Type of parameter", submitOnChange: true  , options: ["text":"string", "number":"number", "decimal":"decimal"]
                    input "ccPval${i}", app.ccPtype$i, title: "Parameter ${i} Value", required: false, description: "Value", submitOnChange: true
                }
            }
        }
    }
...
def getDevTypes(){
	def myMap = [:]
    myMap << ["capability.switch":"Switch"]
    myMap << ["capability.switchLevel":"Dimmer"]
    myMap << ["capability.fan":"Fan"]
}
...
/*/////NEW ADDITION***********************************************************************************************************************************************************************************
            section("Custom Commands"){
                href (name: "ccPage", title: "Custom Stuff 1", 
       				description: "Tap to configure custom command",
       				//image: verImgCheck(childVer), required: false, // check repo for image that matches current version. Displays update icon if missing
       				page: "ccPage"
				)		
