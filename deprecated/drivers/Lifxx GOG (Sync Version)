 ...
 *
 *
 *	Hubitat Version - edited by Stephan Hackett (3/25/18)
 *	- async to sync (redid all httpPost/Put/Get and handlers
 *	- map had to be rebuilt when received by sendLIFXCommand() method
 *	- requestContentType added to headers
 *	- colortemp value.tonteger()
 *	- removed unneeded tile info
 *	- replaced runEvery1Minute with schedule(cron)
 *	- added versioning
 *
 *	4/15/18
 *	- edited Api Effects, setTemp and setLevel methods to adjsut for recent firmware changes
 *
 *	4/18/18
 *	- fixed ApiFlash and ApiBreathe
 *	- added setColor functionality
 *	- commented out webCore custom commands to clean up device page. Can easliy be readded by removing comments
 *
 *
 *
...
def version(){return "v1.0.20180418"}
...
def sendLIFXCommand(commands) {
    def myKeySet = commands.keySet()
    //log.warn myKeySet
    def newCommand = [:]
    if (myKeySet[0]=="color"){
    	myKeySet.each {myKey->
       		newCommand[myKey] = commands."${myKey}".toString()
    	}
        commands = newCommand
    }
    ...
    httpPut(params) { response ->
        if(response.getStatus() == 200 || response.getStatus() == 207) {
        	...
        	def totalBulbs = response.getData().results.size()
        	def results = response.getData().results
        	def bulbsOk = 0
            ...
            //log.info response.getData().results.size()
    	}
        else {
...
def runLIFXEffect(commands, effect) {
    ...
    myKeySet.each {myKey->
        //log.error myKey
        if(myKey.contains("color")){
            newCommand[myKey] = commands."${myKey}".toString()
        }
        else{
            newCommand[myKey] = commands."${myKey}"
        }
    }
    commands = newCommand
